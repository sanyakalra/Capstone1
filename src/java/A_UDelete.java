 ...
 * @author Sanya
 */
@WebServlet(urlPatterns = {"/A_UDelete"})
public class A_UDelete extends HttpServlet {
         ...
         try {
           ...
           s.executeQuery("SELECT * FROM executive where EID = "+request.getParameter("uid"));
             ...
             if(!rs.next())
            {
                     ...
                     out.println("location='Admin_UDelete.html';");
                     out.println("</script>");
            }
             else{
               
                     s.executeUpdate("DELETE from executive where EID="+request.getParameter("uid"));
                     out.println("<script type=\"text/javascript\">");
